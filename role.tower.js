...
var roleTower = {
	...
	run: function(tower) {
		...
		var targetsContainerRepair = tower.room.find(FIND_STRUCTURES, {filter : (hp) => hp.hits < (hp.hitsMax - 800) && hp.hits < 30000000 && hp.structureType == STRUCTURE_CONTAINER});
		var targetsCreepRepair = _.filter(Game.creeps, (creep) => creep.hits < creep.hitsMax);
		targetsMyRepair.push(...targetsContainerRepair)
		...
		// Repair my structures + container while above 50% energy
